...
public class CompoundDistribution extends Distribution {
    ...
    public void initAndValidate() throws Exception {
        ...
        useThreads = useThreadsInput.get() && (YabbyMCMC.m_nThreads > 1);
...
//        }
        ...
        try {
            ...
            for (Distribution dists : pDistributions.get()) {
                if (dists.isDirtyCalculation()) {
                    ...
                    YabbyMCMC.g_exec.execute(coreRunnable);
                }
            }
            m_nCountDown.await();
            logP = 0;
            ...
            // refresh thread pool
            YabbyMCMC.g_exec = Executors.newFixedThreadPool(YabbyMCMC.m_nThreads);
    ...
    public List<YABBYObject> listActivePlugins() throws IllegalArgumentException, IllegalAccessException {
    	if (ignoreInput.get()) {
    		return new ArrayList<YABBYObject>();
    	} else {
