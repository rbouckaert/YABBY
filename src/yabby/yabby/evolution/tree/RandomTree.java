...
//	}
        ...
        for (final YABBYObject plugin : outputs) {
            if (plugin instanceof MRCAPrior && !calibrations.contains(plugin) ) {
                calibrations.add((MRCAPrior) plugin);
            }
        }
        if (m_initial.get() != null) {
            for (final YABBYObject plugin : m_initial.get().outputs) {
                if (plugin instanceof MRCAPrior && !calibrations.contains(plugin) ) {
                    calibrations.add((MRCAPrior) plugin);
                }
            }
     ...
     * @throws yabby.evolution.tree.RandomTree.ConstraintViolatedException
     */
