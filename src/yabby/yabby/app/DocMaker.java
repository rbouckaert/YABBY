...
public class DocMaker {
    ...
    public DocMaker() {
        ...
        m_sPluginNames = AddOnManager.find(yabby.core.YABBYObject.class, AddOnManager.IMPLEMENTATION_DIR);
        ...
        for (String sPlugin : m_sPluginNames) {
            try {
                ...
                YABBYObject plugin = (YABBYObject) _class.newInstance();
...
//                    "		      <FRAME src='http://www.omnomnomnom.com/random/rotate.php' align='center'>\n" +
     ...
     */
    String[] getImplementations(YABBYObject plugin) {
...
//        	buf.append(" no, this cannot be used in a log.");
     ...
     */
    String getType(YABBYObject plugin, String sName) {
        try {
                ...
                if (fields[i].getType().isAssignableFrom(Input.class)) {
                    ...
                    if (input.getName().equals(sName)) {
                        ...
                        if (input.getType() != null) {
                            return (input.getType().isAssignableFrom(YABBYObject.class) ? "<a href='" + input.getType().getName() + ".html'>" : "") +
                                    input.getType().getName() +
                                    (input.get() != null && input.get() instanceof List<?> ? "***" : "") +
                                    (input.getType().isAssignableFrom(YABBYObject.class) ? "</a>" : "");
                        }
