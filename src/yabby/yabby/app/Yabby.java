...
//        browser.setScaleY(1.1);
        ...
        webEngine.load("http://localhost:" + port + "/yabby.html");
//        webEngine.load("http://www.w3schools.com/js/js_timing.asp");
...
//	}
	...
	public String handleRequest(String url, StringBuffer data) throws IOException {
		...
		if (url.startsWith("quitYabby")) {
			...
			HTMLPrintStream.currentLevel = null;
			//if (webEngine != null) {
			//	webEngine.executeScript("$(\"#log\").append(\"" + output + "\")");
			//}
			return output;
...
//				outfile.close();
    ...
    public boolean initialise() throws Exception {
        ...
        File yabbyFile = inputfileInput.get();

        if (yabbyFile != null && yabbyFile.getParent() != null && workingInput.get()) {
            System.setProperty("file.name.prefix", yabbyFile.getParentFile().getAbsolutePath());
        }
        ...
        if (yabbyFile.getName().toLowerCase().endsWith("xml")) {
        	m_runnable = new XMLParser().parseFile(yabbyFile);
        } else {
        	m_runnable = new JSONParser().parseFile(yabbyFile);
        }
        m_runnable.setStateFile(yabbyFile.getName() + ".state", resume);
    	return true;
    } // init
