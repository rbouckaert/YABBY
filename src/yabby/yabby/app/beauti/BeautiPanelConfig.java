...
public class BeautiPanelConfig extends YABBYObject {
...
//                System.err.println("sync " + parentPlugins.get(iPartition) + "[?] = " + _input.get());
            ...
            bIsList = false;
            for (int i = 0; i < sPathComponents.length; i++) {
                List<YABBYObject> oldPlugins = plugins;
                plugins = new ArrayList<YABBYObject>();
                parentPlugins = new ArrayList<YABBYObject>();
                parentInputs = new ArrayList<Input<?>>();
                for (YABBYObject plugin : oldPlugins) {
                    ...
                    if (namedInput.get() instanceof List<?>) {
                        ...
                        if (sConditionalAttribute[i] == null) {
                            for (Object o : list) {
                                YABBYObject plugin2 = (YABBYObject) o;
                                plugins.add(plugin2);
                            ...
                            for (int j = 0; j < list.size(); j++) {
                                YABBYObject plugin2 = (YABBYObject) list.get(j);
                    ...
                    } else if (namedInput.get() instanceof YABBYObject) {
                        bIsList = false;
                        if (sConditionalAttribute[i] == null) {
                            plugins.add((YABBYObject) namedInput.get());
                            parentPlugins.add(plugin);
                            parentInputs.add(namedInput);
                        } else {
...
//							    (m_sConditionalAttribute[i].equals("type") && plugin.getClass().getName().equals(m_sConditionalValue[i]))) {
            ...
            if (!bIsList) {
                _input = new FlexibleInput<YABBYObject>();
            } else {
                _input = new FlexibleInput<ArrayList<YABBYObject>>(new ArrayList<YABBYObject>());
            }
...
//                System.err.println("sync " + parentPlugins.get(iPartition) + "[?] = " + _input.get());
    ...
    private boolean matches(YABBYObject plugin2, String sConditionalAttribute, String sConditionalValue) {
    ...
    public void sync(int iPartition) {
        if (parentInputs.size() > 0) {
            ...
            if (bIsList) {
                ...
                for (YABBYObject plugin : startInputs) {
    ...
    public void syncTo(int iPartition) {
        ...
        try {
            if (bIsList) {
                for (YABBYObject plugin : inputs) {
                    _input.setValue(plugin, this);
                }
            } else {
            	if (inputs.size() > 0) {
	                YABBYObject plugin = inputs.get(iPartition);
	                _input.setValue(plugin, this);
            	}
            }
        } catch (Exception e) {
