...
public class YABBYObjectInputEditor extends InputEditor.Base {
    private static final long serialVersionUID = 1L;
    JComboBox m_selectPluginBox;
    SmallButton m_editPluginButton;

    YABBYObjectInputEditor _this;

    //public PluginInputEditor() {}
    public YABBYObjectInputEditor(BeautiDoc doc) {
        super(doc);
        _this = this;
    }
     ...
     * *
     */
    void simpleInit(Input<?> input, YABBYObject plugin) {
        ...
        if (m_bAddButtons) {
            if (YABBYObjectPanel.countInputs((YABBYObject) m_input.get(), doc) > 0) {
                ...
                m_editPluginButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        YABBYbjectDialog dlg = new YABBYbjectDialog((YABBYObject) m_input.get(), m_input.getType(), doc);
                        if (dlg.showDialog()) {
                            try {
                                dlg.accept((YABBYObject) m_input.get(), doc);
                            } catch (Exception ex) {
                                ex.printStackTrace();
                            }
                        }
    ...
    void refresh() {
    	if (m_selectPluginBox != null) {
	        ...
	        String sID = ((YABBYObject) m_input.get()).getID();
...
//        }
    ...
    void expandedInit(Input<?> input, YABBYObject plugin) {
        addInputLabel();
        ...
        doc.getInpuEditorFactory().addInputs(box, (YABBYObject) input.get(), this, this, doc);
...
//						public void run() {
                    ...
                    YABBYObject plugin = (YABBYObject) m_input.get();
...
//                            m_selectPluginBox.setSelectedItem(sID);
                        ...
                        sID = ((YABBYObject) m_input.get()).getID();
                        m_selectPluginBox.setSelectedItem(sID);
                        //ex.printStackTrace();
