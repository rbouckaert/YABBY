...
public interface InputEditor {
     ...
     */
    void init(Input<?> input, YABBYObject plugin, int itemNr, ExpandOption bExpandOption, boolean bAddButtons);
...
public abstract class Base extends /*Box*/ JPanel implements InputEditor { //, ValidateListener {
     ...
     * parent plugin *
     */
    protected YABBYObject m_plugin;
    /**
...
//   	}
	...
	protected void setValue(Object o) throws Exception {
    	if (itemNr < 0) {
    		...
    		if (other != o) {
    			if (other instanceof YABBYObject) {
    				((YABBYObject)other).outputs.remove(m_plugin);
    			}
    			list.set(itemNr, o);
    			if (o instanceof YABBYObject) {
    				((YABBYObject) o).outputs.add(m_plugin);
    			}
    		}
    	}
    }
            ...
            if (input.get() instanceof YABBYObject) {
                ...
                for (Input<?> input2 : plugin.listInputs()) {
                    try {
                        ...
                        throw new Exception(((YABBYObject) input.get()).getID() + "</p><p> " + e.getMessage());
                    }
                    ...
                    if (o != null && o instanceof YABBYObject) {
                        ...
                        for (Input<?> input2 : plugin.listInputs()) {
                            try {
                                ...
                                throw new Exception(((YABBYObject) o).getID() + " " + e.getMessage());
                            }
                            validateRecursively(input2, done);
                        }
                    }
