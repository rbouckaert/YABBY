...
//                        	doc.close();
    ...
    boolean needsDrawing(YABBYObject plugin) {
        if (plugin == null) {
            return true;
        }
        if (!m_bViewOperators && plugin instanceof yabby.core.Operator) {
            return false;
        }
        if (!m_bViewLoggers && plugin instanceof yabby.core.Logger) {
            return false;
        }
        if (!m_bViewSequences && plugin instanceof Sequence) {
            return false;
        }
        if (!m_bViewState && plugin instanceof yabby.core.State) {
            return false;
        }
        return true;
    }
    ...
    public void setDrawingFlag() {
        for (int i = 0; i < m_doc.m_objects.size(); i++) {
            ...
            if (shape instanceof YABBYObjectShape) {
                YABBYObject plugin = ((YABBYObjectShape) shape).m_plugin;
                if (needsDrawing(plugin)) {
                    shape.m_bNeedsDrawing = true;
                }
            } else if (shape instanceof InputShape) {
                YABBYObjectShape pluginShape = ((InputShape) shape).m_pluginShape;
                if (pluginShape != null) {
                ...
                if (tail instanceof YABBYObjectShape) {
                    bNeedsDrawing = needsDrawing(((YABBYObjectShape) tail).m_plugin);
                }
                if (bNeedsDrawing) {
                    Shape head = ((Arrow) shape).m_headShape;
                    if (head instanceof InputShape) {
                        YABBYObjectShape pluginShape = ((InputShape) head).m_pluginShape;
                        if (pluginShape != null) {
            ...
            public void mouseDragged(MouseEvent me) {
                switch (m_nMode) {
                    ...
                    case MODE_MOVE:
                        ...
                        break;
                    case MODE_FUNCTION:
                        yabby.app.draw.YABBYObjectShape function = (yabby.app.draw.YABBYObjectShape) m_drawShape;

                        if (m_drawShape == null) {
                            function = new yabby.app.draw.YABBYObjectShape();
                    ...
                    case MODE_ARROW:
                        ...
                        if (m_drawShape == null) {
                            ...
                            arrow = new Arrow((YABBYObjectShape) shape, me.getX(), me
                                    .getY());
                            arrow.m_w = 1;
                            arrow.m_h = 1;
                            m_drawShape = arrow;
            ...
            public void mouseMoved(MouseEvent me) {
                ...
                for (int i = m_doc.m_objects.size() - 1; i >= 0; i--) {
                    ...
                    if (shape.m_bNeedsDrawing
                            && shape.intersects(me.getX(), me.getY())) {
                        if (shape instanceof YABBYObjectShape) {
                            YABBYObjectShape plugin = (YABBYObjectShape) shape;
                            try {
                    ...
                    for (int i = m_doc.m_objects.size() - 1; i >= 0; i--) {
                        ...
                        if (shape.m_bNeedsDrawing
                            ...
                            if (shape instanceof InputShape) {
                                ...
                                YABBYObject plugin = ellipse.getPlugin();
                        ...
                        if (shape instanceof YABBYObjectShape) {
                            ...
                            for (Shape shape2 : m_doc.m_objects) {
                                if (shape2 instanceof YABBYObjectShape) {
                                    plugins.add(((YABBYObjectShape) shape2).m_plugin);
                                }
                            }
                            YABBYbjectDialog dlg = new YABBYbjectDialog(plugin, plugin.getClass(), plugins, null);
                            if (dlg.showDialog()) {
                                // add newly created Plug-ins
                                int nNewShapes = 0;
                                if (plugins.size() < YABBYObjectPanel.g_plugins.size()) {
                                    for (YABBYObject plugin2 : YABBYObjectPanel.g_plugins.values()) {
                                        if (!plugins.contains(plugin2)) {
                                            try {
                                                nNewShapes++;
                                                Shape shape2 = new YABBYObjectShape(plugin2, m_doc);
                                                shape2.m_x = 10;
            ...
            public void mouseReleased(MouseEvent me) {
                ...
                switch (m_nMode) {
                    ...
                    case MODE_MOVE:
                        ...
                        return;
                    case MODE_FUNCTION:
                        YABBYObjectShape function = (YABBYObjectShape) m_drawShape;
                        if (function == null) {
                            return;
                        }
