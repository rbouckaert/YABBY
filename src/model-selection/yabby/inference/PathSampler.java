...
public class PathSampler extends yabby.core.Runnable {
	...
	public void initAndValidate() throws Exception {
		...
		if (!mcmc.getClass().equals(MCMC.class)) {
			System.out.println("WARNING: class is not yabby.core.MCMC, which may result in unexpected behavior ");
		}
    	...
    	for (int i = 0; i < Yabby.m_nThreads; i++) {
    		FileOutputStream outStream = (yabby.app.util.Utils.isWindows()?
		...
		for (int i = 0; i < m_nSteps; i++) {
        	...
        	FileOutputStream xmlFile = new FileOutputStream(stepDir.getAbsoluteFile() + "/yabby.xml");
        	...
        	FileOutputStream cmdFile = 
        			(yabby.app.util.Utils.isWindows()?
...
//instead of skipping #threads steps every time.
			if (i >= Yabby.m_nThreads) {
				String copyCmd = (yabby.app.util.Utils.isWindows()
						? "copy " + getStepDir(i - Yabby.m_nThreads) + "\\yabby.xml.state " + getStepDir(i)
						: "cp " + getStepDir(i - Yabby.m_nThreads) + "/yabby.xml.state " + getStepDir(i)
							);
			...
			File script = new File(stepDir.getAbsoluteFile() + 
					(yabby.app.util.Utils.isWindows()? "/run.bat": "/run.sh"));
	...
	protected Distribution extractLikelihood(MCMC mcmc) throws Exception {
	...
	private String sanitise(String property) {
		...
		if (yabby.app.util.Utils.isWindows()) {
...
//				Process p = Runtime.getRuntime().exec(cmd);
    ...
    public void run() throws Exception {
    	if (doNotRun.get()) {
    		...
    		System.out.println("java yabby.inference.PathSampleAnalyser " + m_nSteps + " " + alphaInput.get() + " " + rootDirInput.get() + " " + burnInPercentage);
    		return;
    	}
    	long startTime = System.currentTimeMillis();

		for (int i = 0; i < m_nSteps; i++) {
	    	if (Yabby.m_nThreads > 1) {
	    		int nSteps = Math.min(Yabby.m_nThreads, m_nSteps - i);
	    		...
	    		m_nCountDown.await();	    		
    			for (int j = 0; j < Yabby.m_nThreads && i+j < m_nSteps; j++) {
...
//				}
	...
	/** copy yabby.xml.state file from previous directory **/
	private void copyStateFile(int iFrom, int iTo) throws Exception {
        ...
        InputStream in = new FileInputStream(new File(prevStepDir.getPath() + fileSep + "yabby.xml.state"));
        OutputStream out = new FileOutputStream(new File(stepDir.getPath() + fileSep + "yabby.xml.state"));
